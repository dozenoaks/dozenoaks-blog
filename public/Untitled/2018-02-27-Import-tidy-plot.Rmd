...
[import image data into R](http://dozenoaks.ghost.io/importing-images-into-r/). 
...
This is my strategy to turn an image into tidy data for downstream analysis and
then import the image. First up, load the packages we need. The `tidyverse`
...
Turning the image into a data table is easy enough, in this case we will turn it
...
to do such a thing?!? Well, all these steps I've just walked through can be
...
cover (the resulting plots were originally called joy plots). I had been messing
around with trying to create a version of this plot for some of my kymograph
data, just because I thought it would be visually striking. `ggridges` is far
