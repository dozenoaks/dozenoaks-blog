...
estimating the number of guassians in your sample (not trivial) and can deal
with non-normal distributions. This post deals with something altogether more
practical - getting ggplot2 to plot multiple guassians with different
...
```{r find-gaussians}
my_mix <- normalmixEM(observations$obv, k = 2) # telling it to find two gaussians in the observations
...
`normalmixEM` provides a list of 9 outputs for the guassians it calculates,
...
gaussian line on top of a histogram. Two guassians requires two `stat_functions`
...
things need fixing - plot all the guassians in a single call, convert them to be
...
ggplot(observations, aes(x = obv)) +
  ...
  mapply(
    ...
    n = length(observations$obv), #sample size
